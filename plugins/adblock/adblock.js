    ...
    if (context === "main") {
      ...
      function initializeAdBlocker() {
        ...
        try {
          ...
          log(
            `Setting up blocking for ${allWebContents.length} existing webContents...`
          );
          ...
          log(
            `Loaded ${adDomains.length} ad domains and ${adPaths.length} ad path patterns`
          );
      ...
      console.log("Executing in renderer");

      // Catch window-level errors
      window.addEventListener("error", (event) => {
        if (
          event.message &&
          event.message.includes("AdError 1005") &&
          event.message.includes("Failed to fetch")
        ) {
          console.warn("Ad error detected → skipping track");
      ...
      window.addEventListener("unhandledrejection", (event) => {
        if (
          ...
          event.reason.message.includes("AdError 1005")
        ) {
          console.warn("Ad fetch failed (promise rejection) → skipping track");
          dzPlayer.control.nextSong();
        }
      });

      console.log("AdBlock fix loaded");
    }
  },
};
